...
def run():
    ...
    if result:
        print "detect qemu-kvm failure successfuly"
        recover = recover_service_fail(client, 20)
        if recover:
            return True
        else:
            return False
...
def recover_service_fail(client, detect_time=5):
    ...
    try:
        ...
        print "UT_Recover_Qemu-KVM_Service recover-qemu-Except:" + str(e)
        result = False
    return result
...
def _create_ssh_client(name, default_timeout=1):
    ...
    try:
        ...
        print "UT_Recover_Qemu-KVM_Service create-ssh-Except:%s" % str(e)
        return None
