...
class Cluster(ClusterInterface):
    ...
    def deleteNode(self, node_name):
        ...
        try:
            ...
            message = "Cluster--delete node fail , node maybe not in compute pool please check again! node is %s .The " \
                      "node list is %s.%s" % (
    ...
    def addInstance(self, instance_id, send_flag = True):
        ...
        try:
            ...
            if message:
                ...
                if final_host in self.getAllNodeStr():
                    ...
                    raise Exception("addInstance--live migration fail")
                if send_flag:
        ...
        try:
            ...
            for instance in self.instance_list:
                ...
                if instance.id == instance_id:
                    ...
                    message = "Cluster--delete instance success. this instance is deleted (instance_id = %s)" % \
                              instance_id
                    data = {"cluster_id": self.id, "instance_id": instance_id}
                    result = self.successResult(message, data)
                    logging.info(message)
            ...
            if result is None:
                message = "Cluster--delete instance fail. maybe this instance is not in instance_list(instance_id = " \
                          "%s)" % instance_id
                data = {"cluster_id": self.id, "instance_id": instance_id}
                result = self.failResult(message, data)
                logging.error(message)
    ...
    def checkInstanceExist(self, instance_id):
        ...
        for instance in instance_list:
            if instance.id == instance_id:
                logging.info("Cluster--add_instance-checkInstanceExist success")
                return True
...
if __name__ == "__main__":
    ...
    def isProtected(self, instance_id):
        for instance in self.instance_list[:]:
            if instance.id == instance_id:
                return True
        message = "this instance is  already in the cluster. Instance id is %s. cluster id is %s .instance list is 
        %s" % (
